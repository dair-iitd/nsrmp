...
from datasets.common.program_translator import nsrmseq_to_nsrmqsseq, nsrmtree_to_nsrmseq, break_nsrmtrees, append_nsrmtrees 
...
class ProgramParser(nn.Module):
    ...
    def __init__(self, vocab, gru_hidden_dim, gru_nlayers, word_emb_dim, positional_emb_dim, use_bert = True,  discount=None):
        ...
        if self.use_bert:
            ...
            if self.positional_embedding_dim is not None:
                ...
                self.positional_embedding = LearnedPositionalEmbedding(200,self.positional_embedding_dim)
                self.tot_embedding_dim += self.positional_embedding_dim
            else:
                self.add_module('positional_embedding', None)
            self.encoder = jacnn.GRULayer(self.tot_embedding_dim, self.hidden_dim, gru_nlayers, bidirectional=True, batch_first=True, dropout=0.5)
       
       
        self.decoder = nn.GRUCell(2*self.hidden_dim, 2*self.hidden_dim)
        self.decoder_attention = nn.Sequential(nn.Linear(4*self.hidden_dim, 2*self.hidden_dim),nn.ReLU(), nn.Linear(2*self.hidden_dim,1))
        self.expand = nn.Sequential(nn.Linear(4*self.hidden_dim, 2*self.hidden_dim), nn.ReLU(), nn.Dropout(), nn.Linear(2*self.hidden_dim, 2),  nn.Softmax(dim=-1))
        self.cross_entropy = nn.CrossEntropyLoss(weight=torch.tensor([1.0,1.0]))
    ...
    def encode_sentence(self, sent, sent_length):
        if self.use_bert:
            ...
            hiddens, last_state = self.encoder(f, sent_length)
    ...
    def forward(self,sent,sent_length,max_steps,concept_groups,relational_concept_groups=[],action_concept_groups=[],sample_method='sample',sample_space=None, exploration_rate = 0.3):
        ...
        sample_space = [break_nsrmtrees(trees) for trees in sample_space] if sample_space is not None else sample_space
       
        sent, hiddens, last_state = self.encode_sentence(sent, sent_length)
        concept_masks = defaultdict(list)
        for i,l in enumerate(sent_length):
            s = sent[i]
            concept_masks['attribute_concept'].append((s == self._token2ids("<CONCEPTS>")) )
            concept_masks["relational_concept"].append(s == self._token2ids('<REL_CONCEPTS>'))
            concept_masks['action_concept'].append(s == self._token2ids('<ACT_CONCEPTS>'))
        ...
        sent_id = []
        losses = []
        decoded_sample_space = [] if sample_space is not None else None
        
        for i in range(sent_length.size(0)):
            if sample_method in (SampleMethod.SAMPLE_ENUMERATE,SampleMethod.GROUNDTRUTH):
                prog_length.append([len(sample) for sample in sample_space[i]])
            elif sample_method in (SampleMethod.SAMPLE,):
                x = [0]
                prog_length.append(x)
            
            if sample_method == SampleMethod.SAMPLE:
                all_h, current = hiddens[i], last_state[i].view(1,-1)
                flag = False
                for step in range(max_steps):
                    if flag:
                        break
                    attended_score = F.softmax(self.decoder_attention(torch.cat([current.expand(all_h.shape), all_h], dim = -1)), dim =-1)
                    attended_hiddens = attended_score*all_h
                    decoded_hiddens.append(attended_hiddens)
                    context = (attended_hiddens).sum(dim=-2, keepdims = True)
                    current = self.decoder(context, current)
                    decoded_states.append(current[0])
                    sent_id.append(i)
                    is_expand = self.expand(torch.cat([current,context], dim =-1))
                    x[0] +=1
                    flag = (is_expand.argmax() == 0)

            elif sample_method in (SampleMethod.GROUNDTRUTH,SampleMethod.SAMPLE_ENUMERATE):
                for instance in sample_space[i]:
                    all_h, current = hiddens[i], last_state[i].view(1,-1)
                    flag = False
                    for step in range(max_steps):
                        if flag:
                            break
                        attended_score = F.softmax(self.decoder_attention(torch.cat([current.expand(all_h.shape), all_h], dim = -1)), dim =-2)
                        attended_hiddens = attended_score*all_h
                        decoded_hiddens.append(attended_hiddens)
                        context = (attended_hiddens).sum(dim=-2, keepdims = True)
                        current = self.decoder(context, current)
                        ...
                        sent_id.append(i)
                        is_expand = self.expand(torch.cat([current,context], dim =-1))
                        decoded_sample_space.append(instance[step])
                        if step < len(instance)-1:
                            losses.append(self.cross_entropy(is_expand, torch.tensor([1], device = is_expand.device)))
                        else:
                            losses.append(self.cross_entropy(is_expand, torch.tensor([0], device = is_expand.device)))
                            flag = True

        prog_length = torch.tensor(prog_length).flatten()
        decoded_hiddens = torch.stack(decoded_hiddens)
        decoded_states = torch.stack(decoded_states)
        if sample_method in (SampleMethod.GROUNDTRUTH, SampleMethod.SAMPLE_ENUMERATE):
            loss = torch.stack(losses).mean()
        else:
            loss = torch.tensor([0.0])
        all_programs = self._get_prog(sent_id, decoded_hiddens, decoded_states, concept_masks, 10, concept_groups, relational_concept_groups, action_concept_groups, sample_method, decoded_sample_space )
        start_idx = 0
        programs = []
        for idx, length in enumerate(prog_length):
            this_programs = all_programs[start_idx:start_idx+length.item()]
            prog_desc = dict(scene_id = sent_id[start_idx], program = nsrmtree_to_nsrmseq(append_nsrmtrees([p['program'] for p in this_programs])),
                     log_likelihood = torch.stack([p['log_likelihood'] for p in this_programs]).sum(), discounted_log_likelihood = torch.stack([p['discounted_log_likelihood'] for p in this_programs]).sum())
            programs.append(prog_desc)
            start_idx += length.item()
        return programs, loss



    def _get_prog(self,sent_id, hiddens, last_state, concept_masks, max_depth, concept_groups, relational_concept_groups=[],action_concept_groups=[],sample_method='sample',sample_space=None, exploration_rate = 0.3):
        sample_method = SampleMethod.from_string(sample_method)
        if sample_method in (SampleMethod.SAMPLE, SampleMethod.SAMPLE_UNIFORM):
            assert sample_space is None, 'Sample with sample space support is not implemented.'
        if not self.training:
        ...
        for i in range(hiddens.size(0)):     
            h, f  = hiddens[i], last_state[i]   
            cg_mask = concept_masks['attribute_concept'][sent_id[i]]
            assert cg_mask.sum().item() == len(concept_groups[sent_id[i]])
            cgs = h[cg_mask]
            rcg_mask = concept_masks['relational_concept'][sent_id[i]]
            assert rcg_mask.sum().item() == len(relational_concept_groups[sent_id[i]])
            rcgs = h[rcg_mask]
            ag_mask = concept_masks["action_concept"][sent_id[i]]
            assert ag_mask.sum().item() == len(action_concept_groups[sent_id[i]])
            ags = h[ag_mask]
            info = dict(
                ...
                parameter_embeddings=dict(
                    ...
                    attribute_concept=concept_groups[sent_id[i]],
                    relational_concept=relational_concept_groups[sent_id[i]],
                    action_concept=action_concept_groups[sent_id[i]]
                ), ret_type='return'
            )

            if sample_method in (SampleMethod.SAMPLE, SampleMethod.SAMPLE_UNIFORM):
                info['p'], info['lp'] = dict(), list()
                programs.append((info['p'], info['lp']))  #program, log_prob
                ...
                if len(this_sample_space) == 0:
                    ...
                    for instance in this_sample_space:
                        ...
                        programs.append((this_info['p'], this_info['lp']))  #program, log_prob
        ...
        for current_depth in range(max_depth):
            ...
            for i, info in enumerate(infos):
                ...
                if parameter_type is None:
                    ...
                    p[parameter_type + '_idx'], log_prob = self._get_parameter_attention(
                        outputs[parameter_type][i], info['parameter_embeddings'][parameter_type],
    ...
    def _get_parameter_attention(self, att_key, att_values, idx=None, exploration_rate = 0):
        attention = (att_key.unsqueeze(0) * att_values).sum(dim=-1)
        sample_mask = None
        if idx is not None:
